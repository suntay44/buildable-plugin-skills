...
Agent command entrypoints use `buildable plan "<prompt>" --compact` to omit the duplicate Markdown render while retaining all structured fields and saving the same plan files. TOON is a summary, including phase status, blockers, and requested auth/persistence choices; use the JSON source of truth for omitted details such as full design rules and expected files. Reuse the saved plan for unchanged requirements and summarize decisions in chat instead of repeating every artifact.

Missing explicit files produce a `references: warning` audit entry, including in TOON; they are retained in the plan and never treated as inspected content. Simple exclusions such as “CRM without login” leave auth off; explicit auth flags still take precedence. Classification includes a short tag-match or fallback reason, and tied scores report medium confidence without changing the existing route.

Plans also record lightweight `provenance`: Buildable version, a hash of the selected template/guidance, and explicit input path/size/modification metadata. When `generate` reuses a saved plan, changes produce advisory audit and stderr warnings while keeping accepted decisions. Old plans without provenance still work. This is not attachment content hashing or a recursive directory snapshot; review changed inputs before relying on an old plan. Re-running `plan` deliberately creates a fresh plan, so preserve any manual decisions you want to carry forward.
